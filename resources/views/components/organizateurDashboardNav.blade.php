...
<body class="flex flex-col min-h-screen bg-gray-100">
    ...
    <aside id="sidebar" class="w-60 bg-[#7A38FC] text-white fixed top-0 left-0 h-screen 
                overflow-y-auto z-40 -translate-x-full lg:translate-x-0">
        <div class="p-4 border-b border-[#8F59FF]">
        ...
        <header class="bg-white shadow fixed top-0 right-0 left-0 lg:left-60 z-20">
            <div class="px-6 py-4 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="{{route('profile')}}" class="w-10 h-10 rounded-full bg-[#7A38FC] flex items-center justify-center text-white font-medium text-lg hover:bg-[#6429e5] ">
                        {{ substr(Auth::user()->Firstname, 0, 1) }}
                    </a>

                    <form action="{{route('logout')}}" method="POST">
                        @csrf
                        <button type="submit" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-4 rounded-lg flex items-center">
